...
<style>
    ...
    * {
    ...
    /* Tous les autres styles CSS identiques... */
    .hero-header {
...
@st.cache_resource
def load_model():
    """Charge le modèle XGBoost pré-entraîné avec gestion d'erreur améliorée"""
    try:
        # Vérifier si le fichier existe
        model_path = "models/xgb_nopca_model.json"
        if not os.path.exists(model_path):
            st.error(f"❌ Modèle non trouvé : {model_path}")
            st.info("💡 Assurez-vous que le dossier 'models/' contient le fichier 'xgb_nopca_model.json'")
            return None
...
def load_dataset():
    ...
    try:
        if not os.path.exists("balanced_backward.csv"):
            st.error("❌ Dataset non trouvé : balanced_backward.csv")
            return None
...
def preprocess_input(data):
    """Préprocesse les données d'entrée avec gestion d'erreur"""
    try:
        df = load_dataset()
        if df is None:
            return None
...
if model is None or dataset is None:
    st.error("⚠️ Certains fichiers sont manquants. L'application ne peut pas fonctionner correctement.")
    st.stop()
